...
module.exports = function(botkit, config) {
    ...
    bot.send = function(message, cb) {
        ...
        if (
            ...
            if (message.type === 'command') {
                ...
                slack_message.command = slack_message.text.substring(0, slack_message.text.indexOf(' '));
                slack_message.text = slack_message.text.substring(slack_message.text.indexOf(' ')).trim();
                slack_message.as_user = false;
                delete slack_message.type;

                bot.api.chat.command(slack_message, function (err, res) {
                    if (err) {
                        cb && cb(err);
                    } else {
                        cb && cb(null, res);
    ...
    */
    ...
    bot.command = function (src, resp, cb) {
        var msg = {};
